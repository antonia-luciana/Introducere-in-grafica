...
public:
	...
	void imagine1(double lungime, int nivel, CPunct &p, CVector v)
	{
		...
		else
		{
			...
			p1 = v.getDest(p, sqrt(2 * pow((2.0 / 3.0 * lungime), 2.0)));

			
			v.rotatie(135);
			patrat(lungime / 3.0, p1, v);
			imagine1(lungime / 3.0, nivel - 1, p1, v);
	...
	{
		CVector v(0.0, 1.0);
		CPunct p(1.0/3, -1.0/3);
		CPunct p1 = p;
		v.rotatie(-135);
		p1 = v.getDest(p, sqrt(2 * pow(( lungime), 2.0)));
		v.rotatie(135);

		patrat(lungime*3,p1 , v );
		patrat(lungime, p, v);
...
public:
		...
		CVector v)
	{
		...
		else
		{
			...
			v1.rotatie(-45);
			imagine2(lungime*factordiviziune, nivel - 1, factordiviziune, p2, v1);
			...
			p2 = v.getDest(p1, lungime);
			v.rotatie(45);
			v.deseneaza(p2,lungime/2.0);
			p = v.getDest(p2, lungime/2.0);

			v1.rotatie(-20);
			//imagine2(lungime*factordiviziune, nivel - 1, factordiviziune, p2, v1);
			imagine2(lungime*factordiviziune, nivel - 1, factordiviziune, p, v1);

			v.rotatie(-120);
			v.deseneaza(p2, lungime / 2.0);
			p2 = v.getDest(p2, lungime/2.0);
			v1.rotatie(-100);
			imagine2(lungime*factordiviziune, nivel - 1, factordiviziune, p2, v1);
	...
	void afisare(double lungime, int nivel)
	{
		...
		v.deseneaza(p, 0.25);
		//p = v.getDest(p, 0.25);
		imagine2(lungime, nivel, 0.5, p, v);
	}
};
...
void Display7() {
	...
	if (nivel % 2 == 1) {
		glTranslated(2.85, 0.0, 0.0);
	}

	img.afisare(lg, nivel);
