...
void Display8() {
	...
	double x = 0.0, y = 0.0, angle = M_PI / 3;
	for (int i = 0; i < 6; i++) {
		angle += M_PI / 3;
		glVertex2d(x + 0.75*cos(angle), y + 0.75*sin(angle));
	}
	glEnd();
	...
	for (int i = 0; i < 6; i++) {
		angle += M_PI / 3;
		glVertex2d(x + 0.5*cos(angle), y + 0.5*sin(angle));
	}
	// de completat ...
	glEnd();
	...
	for (int i = 0; i < 6; i++) {
		angle += M_PI / 3;
		glVertex2d(x + 0.48*cos(angle), y + 0.48*sin(angle));
	}
	// de completat ...
	glEnd();
}
